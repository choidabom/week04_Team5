...
# 로직
# 1. 데이터들을 1차원 배열에 담는다.
# 2. 최소 코인 갯수를 저장할 dp 배열을 만들고 max(10001)로 초기화시켜준다.
# 3. 코인 배열의 값을 가져오고
...
# 최소 코인 갯수를 넣을 것이기 때문에 배열에 초기화를 제일 큰 값을 준다.
dp[0] = 0 # k의 최댓값이 10000이기 때문에 dp 리스트를 10001로, 0원을 택하는 방법은 없으므로 dp[0]=0 으로 초기화시킨다.
